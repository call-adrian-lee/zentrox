...
import type { Job, JobStatus } from '../../core/job.models';
...
export class AdminJobsComponent implements OnInit {
  ...
  readonly deleteTarget = signal<Job | null>(null);
  ...
  readonly form = this.fb.group({
    ...
    status: this.fb.control<JobStatus>('draft', { validators: [Validators.required] })
  });
  ...
  startEdit(job: Job): void {
    ...
    this.modalOpen.set(true);
    runInInjectionContext(this.injector, () => {
      afterNextRender(() => this.bootstrapDescriptionEditor(job.description));
    });
  }
  ...
  save(): void {
    ...
    if (id == null) {
      this.api.createJob(body).subscribe({
        ...
        error: () => {
          this.saving.set(false);
          this.saveErrorKey.set('admin.jobsSaveError');
          this.notify.error('admin.noticeSaveFailed');
        }
      });
    } else {
      this.api.updateJob(id, body).subscribe({
        next: () => {
          this.closeModal();
          this.reload();
          this.saving.set(false);
  ...
  quickSetStatus(job: Job, status: JobStatus): void {
  ...
  confirmDelete(): void {
    ...
    if (!job || !this.canConfirmDelete()) return;
  ...
  private plainTextToDescriptionHtml(text: string): string {
    ...
    return parts.map((part) => `<p>${this.escapeHtml(part).replace(/\r?\n/g, '<br>')}</p>`).join('');
  }
   ...
   * Falls back to `#j-desc` when `viewChild` is stale so save is not skipped with an empty `description`.
   */
  private descriptionEditorEl(): HTMLElement | null {
    return this.descriptionEditor()?.nativeElement ?? (typeof document !== 'undefined' ? document.getElementById('j-desc') : null);
  }
